...
def main():
    video_path = os.path.normpath("raw_videos/jumping_jack/2.mp4")
    if not os.path.exists(video_path):
        print(f"Error: {video_path} does not exist.")
        return

    print("Loading MediaPipe models and KNN jumping_jack classifier...")
    ...
    counter = RepCounter(exercise="jumping_jack")
    ...
    out_path = "jumping_jack_output_2.mp4"
    ...
    while cap.isOpened():
        ...
        annotated = annotate_frame(frame.copy(), landmarks, rep_count, "Jumping Jack", counter.last_angle)
        
        out.write(annotated)
        cv2.imshow("Jumping Jack Test (KNN + MediaPipe)", annotated)
